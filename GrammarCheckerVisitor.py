...
class GrammarCheckerVisitor(ParseTreeVisitor):
    ...
    def visitFunction_definition(self, ctx: GrammarParser.Function_definitionContext):
        ...
        for i in range(len(params)):
            llvm_params.append(f"{llvm_type(params[i])} %{i}")

        print(f"define {llvm_type(tyype)} @{name}({params_separator.join(llvm_params)}) ""{\n")
        ...
        # alloca and store
        for i in range(len(params)):
            llvm_param = llvm_params[i].split()
            llvm_param_type = llvm_param[0]
            llvm_param_ir = llvm_param[1]

            print(f"  %{self.arguments_name[i]} = alloca {llvm_param_type}, align 4\n")
            print(f"  store {llvm_param_type} {llvm_param_ir}, {llvm_param_type}* %{self.arguments_name[i]}, align 4\n")
        ...
        for i in range(len(params)):
            ...
            print(
                f"  %{self.next_ir_register} = load {llvm_param_type}, {llvm_param_type}* %{self.arguments_name[i]}, align 4\n")
            self.identifier_and_register_map[self.arguments_name[i]] = self.next_ir_register
    ...
    def visitStatement(self, ctx: GrammarParser.StatementContext):
        if ctx.RETURN() != None:
            ...
            function_type, params, function_cte_value, function_ir_register = self.ids_defined[
                self.inside_what_function]

            if ctx.expression():
                tyype, cte_value, ir_register = self.visit(ctx.expression())
                if cte_value:
                    print(f"  ret {llvm_type(tyype)} {float_to_hex(cte_value) if tyype == Type.FLOAT else cte_value}\n")
                else:
                    print(f"  ret {llvm_type(tyype)} %{self.next_ir_register - 1}\n")

                if function_type == Type.INT and tyype == Type.FLOAT:
                    err("WARNING: possible loss of information returning float expression from int function '" + self.inside_what_function + "' in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")
                elif function_type != Type.VOID and tyype == Type.VOID:
                    err("ERROR: trying to return void expression from function '" + self.inside_what_function + "' in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")
                    exit(-1)
                elif function_type == Type.VOID and tyype != Type.VOID:
                    err("ERROR: trying to return a non void expression from void function '" + self.inside_what_function + "' in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")
                    exit(-1)
        ...
        for i in range(len(ctx.identifier())):
            ...
            if ctx.expression(i) != None:
                ...
                if expr_type == Type.VOID:
                    ...
                    err("WARNING: possible loss of information assigning float expression to int variable '" + name + "' in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")

                print(f"  %{name} = alloca {llvm_type(expr_type)}, align 4")
                if not cte_value:
                    print(
                        f"  store {llvm_type(expr_type)} %{self.next_ir_register - 1}, {llvm_type(expr_type)}* %{name}, align 4\n")
                else:
                    print(
                        f"  store {llvm_type(expr_type)} {float_to_hex(float(cte_value)) if expr_type == Type.FLOAT else cte_value}, {llvm_type(expr_type)}* %{name}, align 4\n")
            else:
            ...
            self.identifier_and_register_map[name] = name
            ...
            if ctx.array_literal(i) != None:
                ...
                for j in range(len(expr_types)):
                    if expr_types[j] == Type.VOID:
                        err("ERROR: trying to initialize void expression to array '" + name + "' at index " + str(
                            j) + " of array literal in line " + str(token.line) + " and column " + str(
                            token.column) + "\n")
                        exit(-1)
                    elif expr_types[j] == Type.FLOAT and tyype == Type.INT:
                        err("WARNING: possible loss of information initializing float expression to int array '" + name + "' at index " + str(
                            j) + " of array literal in line " + str(token.line) + " and column " + str(
                            token.column) + "\n")
            else:
                # unitialized variables now get value 0
    ...
    def visitVariable_assignment(self, ctx: GrammarParser.Variable_assignmentContext):
        ...
        if ctx.identifier() != None:
            ...
            try:
                ...
                err("ERROR: undefined variable '" + name + "' in line " + str(token.line) + " and column " + str(
                    token.column) + "\n")
                exit(-1)
                return
        ...
        # array assignment
            ...
            try:
                ...
                err("ERROR: undefined array '" + name + "' in line " + str(token.line) + " and column " + str(
                    token.column) + "\n")
                exit(-1)
            ...
            if array_index_cte == None:
                ...
                if array_index_cte < 0 or array_index_cte >= array_length:
                    err("ERROR: array '" + name + "' index out of range in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")
                    exit(-1)
                else:
        ...
        if op == '++' or op == '--':
            if cte_value != None:
                ...
                ir_register_aux = ir_register
                ir_register = self.next_ir_register
                self.next_ir_register += 1
                if op == '++':
                    print(
                        f"  %{ir_register} = {'fadd' if tyype == Type.FLOAT else 'add'} {llvm_type(tyype)} %{ir_register_aux}, 1\n")
                if op == '--':
                    print(
                        f"  %{ir_register} = {'fsub' if tyype == Type.FLOAT else 'sub'}  {llvm_type(tyype)} %{ir_register_aux}, 1\n")
                cte_value = None
        else:
            ...
            if cte_value != None:
                ...
                if op == '*=':
                    print(
                        f"  %{ir_register} {'fmul' if tyype == Type.FLOAT else 'mul'} {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '/=':
                    print(
                        f"  %{ir_register} {'fdiv' if tyype == Type.FLOAT else 'div'} {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '+=':
                    print(
                        f"  %{ir_register} {'fadd' if tyype == Type.FLOAT else 'add'} {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '-=':
                    print(
                        f"  %{ir_register} {'fsub' if tyype == Type.FLOAT else 'sub'} {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '=':
        ...
        if len(ctx.expression()) == 0:
            ...
            if ctx.integer() != None:
                ...
                try:
                    tyype, _, cte_value, ir_register = self.ids_defined[name]
                    if not cte_value:
                        ir_register = self.identifier_and_register_map[name]
                except:
                    token = ctx.identifier().IDENTIFIER().getPayload()
                    err("ERROR: undefined variable '" + name + "' in line " + str(token.line) + " and column " + str(
                        token.column) + "\n")
                ...
                if array_index_cte != None:
                    if array_index_cte < 0 or array_index_cte >= array_length:
                        err("ERROR:  array '" + name + "' index out of bounds in line " + str(
                            token.line) + " and column " + str(token.column) + "\n")
                        exit(-1)
                    else:
                        cte_value = cte_values_array[array_index_cte]
            ...
            if ctx.OP != None:  # unary operators
                ...
                if tyype == Type.VOID:
                    err("ERROR: unary operator '" + text + "' used on type void in line " + str(
                        token.line) + " and column " + str(token.column) + "\n")
                    exit(-1)
                elif cte_value != None:
                    if text == '-':
                    ...
                    ir_register = self.next_ir_register
                    self.next_ir_register += 1
                    print(f"  %{ir_register} = sub {llvm_type(tyype)}, 0, {ir_register_aux}\n")
                    cte_value = None
            else:  # parentheses
                tyype, cte_value, ir_register = self.visit(ctx.expression(0))
            ...
            if left_type == Type.VOID or right_type == Type.VOID:
                err("ERROR: binary operator '" + text + "' used on type void in line " + str(
                    token.line) + " and column " + str(token.column) + "\n")
                exit(-1)
            ...
            if text == '*' or text == '/' or text == '+' or text == '-':
                ...
                if left_cte_value != None and right_cte_value != None:
                    ...
                    llvm_left = str(left_cte_value) if left_cte_value is not None else f"%{left_ir_register}"
                    llvm_right = str(right_cte_value) if right_cte_value is not None else f"%{right_ir_register}"
                    ir_register_aux = self.next_ir_register
                    self.next_ir_register += 1
                    if text == '*':
                        print(
                            f"  %{ir_register_aux} = {'fmul' if tyype == Type.FLOAT else 'mul'} {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '/':
                        print(
                            f"  %{ir_register_aux} = {'fdiv' if tyype == Type.FLOAT else 'div'} {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '+':
                        print(
                            f"  %{ir_register_aux} = {'fadd' if tyype == Type.FLOAT else 'add'} {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '-':
                        print(
                            f"  %{ir_register_aux} = {'fsub' if tyype == Type.FLOAT else 'sub'} {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    cte_value = None
                    ir_register = ir_register_aux
            else:
                tyype = Type.INT
                if left_cte_value != None and right_cte_value != None:
                    if text == '<':
    ...
    def visitArray(self, ctx: GrammarParser.ArrayContext):
        ...
        if tyype != Type.INT:
            token = ctx.identifier().IDENTIFIER().getPayload()
            err("ERROR: array expression must be an integer, but it is " + str(tyype) + " in line " + str(
                token.line) + " and column " + str(token.column) + "\n")
            exit(-1)
    ...
    def visitFunction_call(self, ctx: GrammarParser.Function_callContext):
        ...
        try:
            ...
            if len(args) != len(ctx.expression()):
                err("ERROR: incorrect number of parameters for function '" + name + "' in line " + str(
                    token.line) + " and column " + str(token.column) + ". Expecting " + str(len(args)) + ", but " + str(
                    len(ctx.expression())) + " were given" + "\n")
                exit(-1)
        except:
            err("ERROR: undefined function '" + name + "' in line " + str(token.line) + " and column " + str(
                token.column) + "\n")
            exit(-1)
        ...
        for i in range(len(ctx.expression())):
            ...
            if i < len(args):
                if arg_type == Type.VOID:
                    err("ERROR: void expression passed as parameter " + str(
    ...
    def visitArguments(self, ctx: GrammarParser.ArgumentsContext):
        ...
        for i in range(len(ctx.identifier())):
            ...
            ir_register = i
            self.ids_defined[name] = tyype, -1, cte_value, ir_register
            self.arguments_name.append(name)
            params += [tyype]
        return params
