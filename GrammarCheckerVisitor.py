...
class GrammarCheckerVisitor(ParseTreeVisitor):
    ...
    def visitFunction_definition(self, ctx:GrammarParser.Function_definitionContext):
        ...
        for i in range(len(params)):
            ...
            print(f"  %{self.next_ir_register} = load {llvm_param_type}, {llvm_param_type}* %{self.arguments_name[i]}, align 4\n")
    ...
    def visitStatement(self, ctx:GrammarParser.StatementContext):
        if ctx.RETURN() != None:
            ...
            if ctx.expression():
                ...
                if cte_value:
                    print(f"  ret {llvm_type(tyype)} {cte_value}\n")
                else:
                    print(f"  ret {llvm_type(tyype)} %{ir_register}\n")
    ...
    def visitVariable_assignment(self, ctx:GrammarParser.Variable_assignmentContext):
        ...
        if op == '++' or op == '--':
            if cte_value != None:
                ...
                self.next_ir_register += 1
                if op == '++':
                    print(f"  %{ir_register} = add {llvm_type(tyype)} %{ir_register_aux}, 1\n")
                if op == '--':
                    print(f"  %{ir_register} = sub {llvm_type(tyype)} %{ir_register_aux}, 1\n")
                cte_value = None
        else:
            ...
            if cte_value != None:
                ...
                self.next_ir_register += 1
                if op == '*=':
                    print(f"  %{ir_register} mul {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '/=':
                    print(f"  %{ir_register} div {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '+=':
                    print(f"  %{ir_register} add {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '-=':
                    print(f"  %{ir_register} sub {llvm_type(tyype)} {ir_register_aux}, {ir_register - 1}\n")
                elif op == '=':
        ...
        if len(ctx.expression()) == 0:
            ...
            if text == '*' or text == '/' or text == '+' or text == '-':
                ...
                if left_cte_value != None and right_cte_value != None:
                    ...
                    self.next_ir_register += 1
                    if text == '*':
                        print(f"  %{ir_register_aux} = mul {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '/':
                        print(f"  %{ir_register_aux} = div {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '+':
                        print(f"  %{ir_register_aux} = add {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    elif text == '-':
                        print(f"  %{ir_register_aux} = sub {llvm_type(tyype)} {llvm_left}, {llvm_right}\n")
                    cte_value = None
